...
class StockTemplatesTest {
    @Test fun seedsFiveTemplatesOnceWithExpectedExercises() = runBlocking {
        ...
        assertEquals(listOf("Upper A", "Lower A", "Upper B", "Lower B"), templates.map { it.name })
        assertEquals(listOf(7, 7, 8, 7), templates.map { repository.templateLines(it.id).first().size })
        database.close()
    }
}
